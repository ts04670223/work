...
<body>
	...
	<div id="all">
		<div id="title">
			<?=date("m 月 d 日 l");?> | 今日瀏覽: <?=$_SESSION['total']?> | 累積瀏覽:<?=$Total->q("select sum(`total`) from `total`")[0][0];?> <a style="float:right" href="index.php">回首頁</a></div>
		<div id="title2">
...
if(!empty($_SESSION['login'])){
	if($_SESSION['login']=="admin"){
		?>
		歡迎，<?=$_SESSION['login'];?>
		<a href="backend.php"><button>管理</button></a><a href="api/logout.php"><button>登出</button></a>
	<?php
	}else{
		?>
		歡迎，<?=$_SESSION['login'];?>
